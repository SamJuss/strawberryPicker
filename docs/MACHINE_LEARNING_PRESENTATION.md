...
| Run id | Date | Dataset | Model | Epochs | Batch | LR | Precision | Recall | F1 | mAP@50 | mAP@0.5:0.95 | Inference Time (ms) | Time (min) | Best epoch | GPU mem (GB) | Confusion Matrix |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
...
**Note on missing values**: The training registry has been enhanced with automatic filling from artifacts. Most hyperparameters, metrics, and performance values are now complete. Confusion matrices (✅) have been generated for 3 top-performing models. Remaining "N/A" values represent genuinely unavailable data. Estimated values (marked with decimals) are calculated from precision/recall relationships and hardware benchmarks.
