#!/usr/bin/env python3
"""
Train a YOLOv8s model on ripe-only dataset for 50 epochs with improved hyperparameters.
"""
...
def train_and_evaluate():
    ...
    model_size = 'yolov8s.pt'  # Using YOLOv8 small for better accuracy
    epochs = 50
    imgsz = 640
    batch = 16
    workers = 4
    project = 'model/detection'
    name = f'ripe_only_yolov8s_extended_{datetime.now().strftime("%Y%m%d_%H%M%S")}'
    ...
    print(f"Training YOLOv8s on ripe-only dataset (extended run)")
