#!/usr/bin/env python3
"""
Train a YOLOv8m model on ripe-only dataset for 100 epochs with no early stopping.
"""
...
def train_and_evaluate():
    ...
    model_size = 'yolov8m.pt'  # Using YOLOv8 medium for better accuracy
    epochs = 100
    imgsz = 640
    batch = 16
    workers = 4
    project = 'model/detection'
    name = f'ripe_only_yolov8m_100epochs_{datetime.now().strftime("%Y%m%d_%H%M%S")}'
    ...
    print(f"Training YOLOv8m on ripe-only dataset (100 epochs, no early stopping)")
    ...
    # Train with improved hyperparameters and no early stopping
    results = model.train(
        data=data_yaml,
        epochs=epochs,
        imgsz=imgsz,
